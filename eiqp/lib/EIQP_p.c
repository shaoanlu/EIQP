...
int EIQP(double *Q_copy, double *c_copy, double *A_copy, double *b_copy, double epsilon, int nc, int nb, double *z)
{
    int status;
    ...
    cblas_dgemv(CblasRowMajor,CblasNoTrans,nc,nc,fone,Q,nc,z,ione,fzero,Qz,ione); // Qz = Q*z;
    cblas_dgemv(CblasRowMajor,CblasTrans,nb,nc,fone,A,nb,y,ione,fzero,ATy,ione);  // ATy = A'*y;
    cblas_dgemv(CblasRowMajor,CblasNoTrans,nb,nc,fone,A,nb,z,ione,fzero,Az,ione);  // Az = A*z;
    zTc = cblas_ddot(nc,z,ione,c,ione); // zTc = z'*c;
	yTb = cblas_ddot(nb,y,ione,b,ione); // yTb = y'*b;
	zTQz = cblas_ddot(nc,z,ione,Qz,ione); // zTQz = z'*Q*z;
    ...
    tmp = 1.0/sigma;
	cblas_dscal(nc_nc,tmp,Q,ione);
	cblas_dscal(nc,tmp,c,ione);
	cblas_dscal(nb_nc,tmp,A,ione);
	cblas_dscal(nb,tmp,b,ione);
	cblas_dscal(nc,tmp,Qz,ione);
	cblas_dscal(nc,tmp,ATy,ione);
	cblas_dscal(nb,tmp,Az,ione);
	zTc = tmp*zTc;
	yTb = tmp*yTb;
	zTQz = tmp*zTQz;
	...
	{
		for(j=0;j<nc;j++)
			M0[i*n+j] = Q[i+j*nc]; // Q[i*nc+j];
		for(j=nc;j<nc+nb;j++)
			M0[i*n+j] = A[i+(j-nc)*nc]; // A[i*nb+(j-nc)];
		M0[i*n+(nc+nb)] = -c[i];
	}
	for(i=nc;i<nc+nb;i++)
	{
		for(j=0;j<nc;j++)
			M0[i*n+j] = -A[j+(i-nc)*nc]; // -A[j*nb+(i-nc)];
		M0[i*n+(nc+nb)] = b[i-nc];
	}
	for(j=0;j<nc;j++)
		M0[(nc+nb)*n+j] = c[j];
    ...
    for(iter=1;iter<=max_iter;iter++)
    {
    	...
    	info = LAPACKE_dgesv(LAPACK_COL_MAJOR,n,ione,M,n,ipiv,u,n);
		...
		cblas_dscal(nc,gamma,r_z,ione);
		cblas_dscal(nb,gamma,r_y,ione);
		r_tau = gamma*r_tau;
    }

    if(kappa<tau)
    {
        status = 1; // denotes: find the optimal solution
    	for(i=0;i<nc;i++)
    		z[i] = z[i]/tau;
    }
    else
    {
        status = 0; // denotes: detect the infeasibility
    	for(i=0;i<nc;i++)
    		z[i] = 0.0;
    }
    free(M0); free(M);
